...
from .column import column_schema_builder
from .index import index_schema_builder
from .foreignkey import foreign_key_schema_builder
from .option import SchemaOption

REGEX_MULTI_SPACE = re.compile("\s\s+")


def table_schema_builder(database, tables=None):
    """
    ...
    if tables is None:
        ...
        table_lst = [f'"{tb}"' for tb in table_lst]
        table_lst = ", ".join(table_lst)
        sql = f"""
                SELECT TABLE_NAME, ENGINE, ROW_FORMAT, AUTO_INCREMENT,
                        CREATE_OPTIONS, TABLE_COLLATION, TABLE_COMMENT
                from information_schema.`TABLES`
                WHERE TABLE_SCHEMA='{database.name}' AND TABLE_NAME IN ({table_lst})
                AND not isnull(ENGINE)
            """
    ...
    for table_info in tables:
        ...
        table.options["row_format"] = SchemaOption(
    ...
    * COMMENT  == ``options['comment']``
    ...
    def create(self):
        ...
        result = cursor.execute(
            "SHOW CREATE TABLE `%s`.`%s`" % (self.parent.name, self.name)
        )
    ...
    def __eq__(self, other):
        ...
        return (
            (self.options["engine"] == other.options["engine"])
