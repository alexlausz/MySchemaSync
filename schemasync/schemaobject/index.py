...
def index_schema_builder(table):
    ...
    indexes = conn.execute(
        "SHOW INDEXES FROM `%s`.`%s`" % (table.parent.name, table.name)
    )
    ...
    for index in indexes:
        ...
        if index["Column_name"] not in idx[n].fields:
            idx[n].fields.insert(
                index["Seq_in_index"], (index["Column_name"], index["Sub_part"] or 0)
            )
...
class IndexSchema(object):
    ...
    def create(self):
        ...
        sql.append(
            "(%s)" % ", ".join([self.format_sub_part(f, l) for f, l in self.fields])
        )
